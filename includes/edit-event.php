...
if (isset($_FILES['file']) && file_exists($_FILES['file']['tmp_name']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
  ...
  if (in_array($detectedType, $allowedTypes)) {
    if (move_uploaded_file($_FILES['file']['tmp_name'], "/var/www/uploads/calendar/event{$edit_id}")) {
      shell_exec("convert /var/www/uploads/calendar/event{$edit_id} -define jpeg:extent=128kb /var/www/uploads/calendar/thumbnail{$edit_id}"); // https://stackoverflow.com/a/11920384/2624391
      $query .= ", has_img = 1";
    }
  }
}
